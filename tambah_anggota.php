...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nim === '' || $nama === '' || $email === '') {
        ...
        $check = $pdo->prepare('SELECT id FROM anggota WHERE nim = ?');
        $check->execute([$nim]);
        if ($check->fetch()) {
            $error = 'NIM sudah terdaftar.';
        } else {
            $stmt = $pdo->prepare('INSERT INTO anggota (nim, nama_lengkap, email, no_telepon, status, tgl_daftar) VALUES (?, ?, ?, ?, ?, ?)');
            $stmt->execute([$nim, $nama, $email, $telepon, $status, $tglDaftar]);
            header('Location: dashboard.php?msg=' . urlencode('Anggota baru berhasil ditambahkan.'));
            exit;
        }
    }
}
...
<title>Tambah Anggota - Manajemen Anggota</title>
    ...
    <main class="main-content">
        <div class="page-header">
            <div>
                ...
                <p>Isi form berikut untuk menambahkan anggota baru</p>
            </div>
        </div>
            ...
            <form method="POST" action="tambah_anggota.php">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="tgl_daftar" class="form-control" value="<?= htmlspecialchars($_POST['tgl_daftar'] ?? date('Y-m-d')) ?>">
                    </div>
                </div>
